                    ...
                    {challans.map((challan, index) => (
                      <tr key={challan.challanNumber} className={`hover:bg-gray-50 transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-gray-25'}`}>
                        ...
                        <td className="px-6 py-4 text-sm text-center text-gray-900 whitespace-nowrap">
                          ...
                          <div className="text-xs text-gray-500">{challan.createdAt ? format(new Date(challan.createdAt), 'hh:mm a') : ''}</div>
                        </td>
                ...
                challans.map(challan => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <span className={`inline-flex items-center gap-1 px-2 py-1 sm:px-2.5 rounded-full text-xs font-medium ${activeTab === 'udhar' ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}>
                      ...
                      <div className="flex flex-wrap gap-3">
                        <div className="flex items-center gap-1.5 sm:gap-2 text-gray-700">
                          ...
                          <div className="flex flex-col sm:flex-row sm:gap-1.5 sm:items-baseline">
                            ...
                            <span className="text-xs text-gray-400">{challan.createdAt ? format(new Date(challan.createdAt), 'hh:mm a') : ''}</span>
                          </div>
                        </div>
                    ...
                    <div className="grid grid-cols-4 gap-1.5 sm:gap-2">
                      {[
                        ...
                        >
                          {icon}
                          <span className="text-[11px] sm:text-xs font-medium">{t(labelKey)}</span>
                        </button>
                      ))}
                    </div>
                  </div>
                      ...
                      {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                        ...
                        return (
                          <button
                            ...
                            className={`px-2.5 sm:px-3 py-1 sm:py-1.5 text-xs sm:text-xs lg:text-sm font-medium rounded-lg transition-colors touch-manipulation active:scale-95 ${
                              currentPage === pageNum
